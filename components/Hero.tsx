...
import { IconArrowRight, IconSearch, IconFileDown, IconMapPin, IconActivity, IconZap, IconShield, IconGlobe, IconChevronDown, IconLayers } from './Icons';
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 text-center relative z-10 w-full">
        ...
        >
          ...
          >
              ...
              <motion.div
                  layout
                  className={`relative group px-6 py-3 cursor-pointer rounded-3xl md:rounded-l-full transition-all duration-300 ${focusedField === 'keyword' ? 'bg-white shadow-xl dark:bg-slate-800 scale-105 z-40 ring-2 ring-primary-500/20' : 'hover:bg-slate-50 dark:hover:bg-white/5'}`}
                  onClick={() => setFocusedField('keyword')}
                  style={{
                      flex: focusedField === 'keyword' ? 1.4 : 1,
                  }}
              >
                  <div className="flex flex-col items-start justify-center h-full relative">
                        <div className="flex items-center gap-2 mb-1">
                            <label className={`text-[10px] font-extrabold tracking-widest uppercase transition-colors ${focusedField === 'keyword' ? 'text-primary-500' : 'text-slate-400 dark:text-slate-500'}`}>Target Industry</label>
                        </div>
                        <div className="flex items-center gap-3 w-full">
                           <div className={`p-1.5 rounded-lg transition-colors ${focusedField === 'keyword' ? 'bg-primary-50 text-primary-600 dark:bg-primary-900/20 dark:text-primary-400' : 'bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500'}`}>
                             <IconLayers className="w-4 h-4" />
                           </div>
                           <div className="flex-1 min-w-0">
                             <div className={`text-base font-bold truncate ${keyword ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500 opacity-60'}`}>
                                {keyword || "Select Industry..."}
                             </div>
                           </div>
                           <IconChevronDown className={`w-4 h-4 transition-transform duration-300 ${focusedField === 'keyword' ? 'rotate-180 text-primary-500' : 'text-slate-400 dark:text-slate-500'}`} />
                        </div>
                  </div>
                  <AnimatePresence>
                    {focusedField === 'keyword' && (
                      <motion.div
                        ...
                        animate="visible"
                        exit="exit"
                        className="absolute top-[120%] left-0 w-[160%] min-w-[360px] max-h-[450px] overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-2xl z-50 text-left flex flex-col"
                      >
                         ...
                         <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 backdrop-blur-sm sticky top-0 z-20">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl py-3 pl-10 pr-4 text-sm font-semibold text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-primary-500/20 transition-all outline-none"
                                    value={keyword}
                            ...
                            {filteredCategories.length > 0 ? (
                              ...
                              {filteredCategories.map((item) => (
                                <motion.button
                                  ...
                                  className="w-full text-left px-3 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-2xl transition-all flex items-center gap-3 group/item border border-transparent hover:border-slate-200 dark:hover:border-slate-700"
                                  whileHover={{ scale: 1.02 }}
                                  whileTap={{ scale: 0.98 }}
                                >
                                  <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 group-hover/item:text-white group-hover/item:bg-primary-500 transition-all shadow-sm group-hover/item:shadow-md shadow-slate-200 dark:shadow-none shrink-0">
                                    {item.icon}
                                  </div>
                              ...
                              <div className="px-4 py-8 text-center bg-slate-50 dark:bg-slate-800/50 rounded-2xl mx-2 border border-slate-100 dark:border-slate-800 border-dashed">
                                <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">No specific category found</p>
                                <p className="text-xs text-slate-400 mb-4">You can still search for "{keyword}"</p>
                                <button
                                  type="button"
                                  onClick={() => { setFocusedField(null); inputRefCity.current?.focus(); setFocusedField('city'); }}
                                  className="text-xs bg-white dark:bg-slate-700 text-slate-900 dark:text-white px-4 py-2 rounded-lg font-bold border border-slate-200 dark:border-slate-600 hover:border-primary-500 hover:text-primary-500 transition-colors shadow-sm"
                                >
                                  Use "{keyword}" →
                                </button>
                              </div>
